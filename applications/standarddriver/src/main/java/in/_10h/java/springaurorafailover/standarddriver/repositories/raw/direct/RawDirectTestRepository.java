...
public interface RawDirectTestRepository {
    ...
    @Select("SELECT `id`, `text_val`, `int_val` FROM `test`")
    @Results(
        id = "test",
        value = {
            @Result(column = "id", property = "id", id = true),
            @Result(column = "text_val", property = "textVal", id = false),
            @Result(column = "int_val", property = "intVal", id = false),
        }
    )
    List<Test> findAll();
    ...
    @Insert("INSERT INTO `test` (`text_val`, `int_val`) VALUES (#{textVal}, #{intVal})")
